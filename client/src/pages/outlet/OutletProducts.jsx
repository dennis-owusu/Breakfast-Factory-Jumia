...
const OutletProducts = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        ...
        const response = await fetch('/api/route/allcategories', { headers });
        if (!response.ok) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      try {
        ...
        const queryParams = new URLSearchParams({
          ...
          sort: sortBy.includes('price') ? 'productPrice' : sortBy.includes('name') ? 'productName' : sortBy.includes('category') ? 'category' : sortBy.includes('stock') ? 'numberOfProductsAvailable' : 'updatedAt',
        ...
        const response = await fetch(`/api/route/allproducts?${queryParams}`, { headers });
        if (!response.ok) {
          throw new Error(`HTTP error ${response.status}: ${response.statusText}`);
        }
        const contentType = response.headers.get('content-type');
    ...
    fetchProducts();
  }, [searchTerm, sortBy, categoryFilter, currentPage, currentUser?.token]);
  ...
  // Remove the redundant fetchOneProduct useEffect as it's unnecessary
  // Product data is already available in productToDelete
              ...
              <SelectContent>
                ...
                {Array.isArray(categories) &&
                  categories.map((category) => (
                    <SelectItem key={category._id} value={category.stringId}>
                      {category.name}
                    </SelectItem>
                  ))}
              </SelectContent>
                  ...
                  <tr>
                    ...
                    >
                      <div
                        className="flex items-center cursor-pointer"
                        onClick={() =>
                          handleSortChange(sortBy === 'category-asc' ? 'category-desc' : 'category-asc')
                        }
                      >
                        Category
                        <ArrowUpDown className="h-4 w-4 ml-1" />
                  ...
                  {products.map((product) => (
                    <tr key={product._id} className="hover:bg-gray-50">
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">
                        ...
                        >
                          {product.numberOfProductsAvailable > 0
                            ? `${product.numberOfProductsAvailable} in stock`
                            : 'Out of stock'}
                        </span>
                      </td>
                    ...
                    {Array.from({ length: totalPages }, (_, i) => i + 1).map((page) => (
                      <Button
                        ...
                        className={`relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium ${
                          currentPage === page ? 'text-orange-600 bg-orange-50' : 'text-gray-700 hover:bg-gray-50'
                        }`}
          ...
          <div className="fixed inset-0 bg-black/80 bg-opacity-50 flex items-center justify-center z-50">
            <div className="bg-white rounded-lg p-6 max-w-md w-full">
              ...
              <p className="text-sm text-gray-500 mb-6">
                ...
                <span className="font-medium">{productToDelete?.productName}</span>? This action cannot be
                undone.
              </p>
